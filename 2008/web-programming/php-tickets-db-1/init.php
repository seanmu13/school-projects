...
#---------------------------------------------------------------------------------------------------------
	...
	echo "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\"><html><head></script><link rel = 'stylesheet' type = 'text/css' href = 'style3.css' /><title>Init</title></head><body><center>";
	...
	$query[] = "CREATE TABLE admins (id int NOT NULL AUTO_INCREMENT primary key, name varchar(30) NOT NULL, pass varchar(50) NOT NULL)";
	...
	$file = file("admins.txt") or exit("<b>Unable to open admins.txt!</b>");

	foreach($file as $value):
		$line = explode("|",$value);
		$line[2] = trim($line[2]);
		$pass = crypt($line[2],"salt");
		$query[] = "INSERT INTO admins VALUES('$line[0]','$line[1]','$pass')";
	endforeach;
